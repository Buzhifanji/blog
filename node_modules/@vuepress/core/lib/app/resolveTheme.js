...
const resolveTheme = (app, themeName) => {
    ...
    const themeEntry = (0, utils_1.requireResolve)(utils_1.path.isAbsolute(themeName)
        ? themeName
        : (0, shared_1.normalizePackageName)(themeName, 'vuepress', 'theme'));
    if (themeEntry === null) {
        throw utils_1.logger.createError(`theme is not found: ${utils_1.chalk.magenta(themeName)}`);
    }
    // resolve theme from theme entry
    const theme = (0, resolvePlugin_1.resolvePlugin)(app, themeEntry, app.options.themeConfig);
    return theme;
};
